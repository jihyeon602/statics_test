...
def main():
    # 데이터를 불러옵니다.
    x_train, y_train, x_val, y_val = preprocess.load_data('q4\data\iris.csv', independent_var = ['petal length', 'sepal length'], response_var = 'class_num')
    ...
    print("mean accuracy for linear kernel: {}%".format(mean_acc_linear*100)) #70
    print("mean accuracy for poly kernel: {}%".format(mean_acc_poly*100)) #70
    print("mean accuracy for rbf kernel: {}%".format(mean_acc_rbf*100)) #80
    print("mean accuracy for sigmoid kernel: {}%".format(mean_acc_sig*100)) #40
